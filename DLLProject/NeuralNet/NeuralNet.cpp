...
void NeuralNet::ApplyMutations(float _probability, float _maxOffsetAmount)
{
	...
	for (int i = 0; i < numConnectionSets; i++)
	{
		...
		if (randomNumber <= _probability)
		{
			...
			weightMatrices[i] = MutateMatrix(weightMatrices[i], _maxOffsetAmount);
			biasMatrices[i] = MutateMatrix(biasMatrices[i], _maxOffsetAmount);
		}
	}
}
...
{
	// Convert the matrix into an array type so we can perform an operation to each of the elements individually
	Eigen::ArrayXf matrixArray = _mat.array();

	// Loop through the matrix array and apply the activation function to all of its elements individually
	for (int row = 0; row < matrixArray.rows(); row++)
	{
		for (int col = 0; col < matrixArray.cols(); col++)
		{
			...
			matrixArray(row, col) = Sigmoid(matrixArray(row, col));
		}
	}

	// Return the "activated" matrix
	return matrixArray.matrix();
}
...
float NeuralNet::Mutate(float _x, float _maxOffsetAmount)
{
	...
	float mutationAmount = RandomInRange(-_maxOffsetAmount, _maxOffsetAmount);
...
Eigen::MatrixXf NeuralNet::MutateMatrix(Eigen::MatrixXf _mat, float _maxOffsetAmount)
{
	// Convert the matrix into an array type so we can perform an operation to each of the elements individually
	//Eigen::ArrayXf matrixArray = _mat.array();
